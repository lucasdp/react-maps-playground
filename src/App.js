...
const Map = ReactMapboxGl({
  accessToken: "pk.eyJ1IjoibHVjYXNkcCIsImEiOiJjaml2dXNiaGswY3NpM2tvc3k2YzR1YWt5In0.qco86APyc1wZPEOt9ZcbeA"
});
...
class App extends Component {
  ...
  onMouseMove = (map, evt) => {
    ...
    if (this.state.movingDot !== -1 && this.state.movingDot !== 0) {
      gjLocal.features[0].geometry.coordinates[0][this.state.movingDot] = ([evt.lngLat.lng, evt.lngLat.lat])
    ...
    if (this.state.movingDot === 0 || this.state.movingDot === cantidadDePuntos - 1) {
      ...
      gjLocal.features[0].geometry.coordinates[0][cantidadDePuntos - 1] = ([evt.lngLat.lng, evt.lngLat.lat])
  ...
  OnMouseDown = (evt) => {
    ...
    const coordenadas = geojson.features[0].geometry.coordinates[0]

    coordenadas.forEach((coor, index) => {
      let proximo = index + 1
      if (proximo === coordenadas.length) {
        proximo = 0
      }

      // Transformar segmento en rectangulo

      // const lowerXBound = Math.min(coor[0], coordenadas[proximo][0]);
      // const upperXBound = Math.max(coor[0], coordenadas[proximo][0]);
      // const lowerYBound = Math.min(coor[1], coordenadas[proximo][1]);
      // const upperYBound = Math.max(coor[1], coordenadas[proximo][1]);

      const estado = evt.lngLat.lat >= lowerXBound && evt.lngLat.lat <= upperXBound && evt.lngLat.lng >= lowerYBound && evt.lngLat.lng <= upperYBound;
      console.log(estado);
  ...
  handleClick = async () => {
    ...
    const uriNom = encodeURI(`https://nominatim.openstreetmap.org/search?format=json&street=${street}&addressdetails=1&city=${city}&country=${country}&county=${county}&state=${state}&state_district=${state_district}`);
    console.log(uriNom);

    const uriGoogle = encodeURI(`https://maps.googleapis.com/maps/api/geocode/json?address=${address},${city}&components=locality:${city}&key=AIzaSyB7La5W_I6lHcmai8_8Y15SDyzEBL-CV1c`)
    ...
    fetchAsync().then(res => {
      ...
      if (res.results[0].types[0] === "street_address") {
        punto = [parseFloat(res.results[0].geometry.location.lng), parseFloat(res.results[0].geometry.location.lat)]
        setPunto(zona, punto)
      } else {
      ...
      <div className="App">
        ...
        <Map
          ...
          <GeoJSONLayer
            ...
            fillPaint={{ 'fill-color': { type: 'identity', property: 'fill' }, 'fill-opacity': { type: 'identity', property: 'fill-opacity' } }}
            circleLayout={{ visibility: 'visible' }}
            circlePaint={{ 'circle-color': { type: 'identity', property: 'fill' } }}
            fillOnMouseDown={this.OnMouseDown}
            linelayout={{ visibility: 'visible' }}
            linePaint={{ 'line-color': { type: 'identity', property: 'fill' }, 'line-opacity': 0.8, 'line-width': 2 }}
